...
def replace_text_with_clipboard(hwp, search_text, replacement_type):
    try:
        occurrences = 0
        while True:
            hwp.MovePos(2)  # Move cursor to the start of the document
            ...
            result = hwp.HAction.Execute(
                "RepeatFind", hwp.HParameterSet.HFindReplace.HSet
            )
            if not result:
                break
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Replace text in a HWP file with clipboard content."
    )
    parser.add_argument("file_path", help="Path to the HWP file")
    parser.add_argument(
        "replacements", nargs="+", help="Pairs of search text and replacement file path"
    )
    ...
    if len(args.replacements) % 2 != 0:
        parser.error(
            "Replacements should be in pairs of search text and replacement file path."
        )

    replacements = [
        (args.replacements[i], args.replacements[i + 1])
    ...
    try:
        ...
        for search_text, replacement_file_path in replacements:
            ...
            if content:
                ...
                if replace_text_with_clipboard(
                    hwp, search_text.strip('"'), content_type
                ):
                    print(
                        f"Replaced all instances of '{search_text}' with content from {replacement_file_path}"
                    )
                else:
